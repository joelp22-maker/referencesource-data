...
**Licence position for this dataset.** Factual compilation with source citations. Neck-finish dimensions are facts about physical measurements transcribed by vendors from GPI/SPI standards; short attributed quotes link back. Corrugated grade data is factual product information published by distributors. Pallet dimensions are widely published facts. No standard text is reproduced.

---

Snapshot of [referencesource.org](https://referencesource.org/packaging-material-specification-equivalence/), which is canonical and re-verified on a schedule. If a record here is wrong, that is worth more to us than one that is right — please open an issue.
