...
**Licence position for this dataset.** CAMEO Chemicals is published by NOAA, a US Government agency; its datasheets are US Government works and not copyrightable. The ratings themselves are safety classifications, i.e. facts. The NFPA 704 standard document is not reproduced and must not be — only the per-chemical rating and the datasheet's own short attributed wording.

---

Snapshot of [referencesource.org](https://referencesource.org/nfpa-704-hazard-ratings/), which is canonical and re-verified on a schedule. If a record here is wrong, that is worth more to us than one that is right — please open an issue.
